...
void AckermanEKF::predict(float dt, float theta_imu, float theta_gps_lp) {
    // Matrizes estáticas para evitar stack overflow em ESP32
    // Alocadas uma única vez na memória estática, reutilizadas a cada chamada
    static float A[AEKF_N * AEKF_N];
    static float Phi[AEKF_N * AEKF_N];
    static float PhiP[AEKF_N * AEKF_N];
    static float PhiPPhiT[AEKF_N * AEKF_N];
...
void AckermanEKF::updateCore(const float *z, const float *h,
    ...
    // Buffers estáticos para evitar stack overflow
    static float PHiT[AEKF_N];
    static float K[AEKF_N];
    static float HiP[AEKF_N];
