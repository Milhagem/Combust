...
float Lambda::analisaLambda(){
  ...
  float valorInicial = analogRead(pinLambda);
  ...
  if (Lambda_bruto < valorLambdaMin) {
    Lambda_bruto = valorLambdaMin;
  }
  
  if (Lambda_bruto > ValorLambdaMax) {
    Lambda_bruto = ValorLambdaMax;
  }
